...
function Table({
  ...
  const validateInput = (value, key) => {
    ...
    if (title === "Sample Specifications") {
      switch (key) {
        case "col1": case "col3": case "col4": case "col7":
          return /^[a-zA-Z\s\-]*$/.test(value);
        case "col2": case "col5": case "col6": case "col8":
        case "col9": case "col10": case "col11":
          return /^[0-9]*\.?[0-9]*$/.test(value);
        default:
          return true;
      }
  ...
  const getValidationMessage = (key) => {
    if (title === "Program Specifications") {
      switch (key) {
        case "program": return "Only letters and spaces allowed";
        case "pc_set": case "w_in": case "w_cm": case "l_in":
        case "l_cm": case "wt_unit": case "gsm" : case "unit_carton":
          return "Only numbers and decimals allowed";
        case "style": case "inner_pack":
  ...
  const renderHeaderCell = (header, index) => (
    <th
      key={index}
      className={`p-3 text-left font-medium border-r border-[#2a4d75] last:border-r-0 whitespace-nowrap align-middle
  ${header.key === "fold" ? "w-45" : ""}
  ${header.tall ? "min-w-[180px]" : ""}
`}
      style={{
        width: header.width || undefined,
        minHeight: header.minHeight || undefined,
        height: header.height || undefined, // only if you want fixed height
      }}
    >
  ...
  const renderCell = (
    ...
    tall = false,       // ← add this param
  ) => {
    const isValid = validateInput(value, key);
    const validationMessage = getValidationMessage(key);

    const sharedClass = `w-full px-2 py-1 text-xs border rounded resize-none
    ${readOnly
      ...
      <div className="relative">
        {tall ? (
          <textarea
            ...
            disabled={readOnly}
            rows={3}
            className={`${sharedClass} min-h-[72px]`}
          />
        ) : (
          <input
            type="text"
                    ...
                    {group.variants.map((variant, vIdx) => (
                      <tr key={vIdx} className="hover:bg-gray-50">
                        {headers.map((header, hIdx) => {
                          ...
                          // Actions column (only first row of group)
                          if (key === "actions") {
                            if (vIdx === 0) {
                              return (
                                <td
                                  key={hIdx}
                                  rowSpan={rowSpan}
                                  className="p-2 align-middle border-l border-gray-100 bg-white"
                                  style={{
                                    width: header.width || undefined,
                                    minHeight: header.minHeight || undefined,
                                  }}
                                >
                                  {!readOnly && (
                                    <div className="flex flex-col gap-3 items-center justify-center">
                                      <button onClick={() => onAddTable?.(groupIdx)} title="Add new group below">
                                        <svg className="w-6 h-6 text-[#0f3460]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                      </button>
                                      <button onClick={() => onCopyTable?.(groupIdx)} title="Copy group">
                                      ...
                                      <button onClick={() => onDeleteTable?.(groupIdx)} title="Delete group">
                                      ...
                                      <button onClick={() => onAddRow?.(groupIdx)} title="Add new style to group">
                          ...
                          // Style column with delete button
                          if (key === "style") {
                            return (
                              <td
                                key={hIdx}
                                className="p-3 border-r border-gray-100 align-middle"
                                style={{
                                  width: header.width || undefined,
                                  minHeight: header.minHeight || undefined,
                                }}
                          ...
                          // Group-level fields (rowSpan)
                          if (group.hasOwnProperty(key) && vIdx === 0) {
                            const isTall = header.tall === true;        // ← check header flag
                            return (
                              <td
                                key={hIdx}
                                rowSpan={rowSpan}
                                className={`p-3 border-r border-gray-200 align-middle text-center font-medium bg-gray-50 ${isTall ? "min-w-[220px]" : ""}`}
                              >
                                {renderCell(
                                  group[key],
                          ...
                          if (variant.hasOwnProperty(key)) {
                            const unit =
                              key === "w_in" || key === "l_in" ? "inches" :
                                key === "w_cm" || key === "l_cm" ? "centimeter" : "";

                            return (
                              <td
                                key={hIdx}
                                className="p-3 border-r border-gray-100 align-middle text-center"
                                style={{
                                  width: header.width || undefined,
                                  minHeight: header.minHeight || undefined,
                                }}
                          ...
                          return (
                            <td
                              key={hIdx}
                              className="p-3 border-r border-gray-100 text-center"
                              style={{
                                width: header.width || undefined,
                                minHeight: header.minHeight || undefined,
                              }}
                    ...
                    {headers.map((header, hIdx) => {
                      ...
                      if (key === "actions") {
                        return (
                          <td
                            key={hIdx}
                            className="p-3 border-r border-gray-100 text-center w-16"
                            style={{
                              width: header.width || "60px",
                              minHeight: header.minHeight || undefined,
                            }}
                          >
                            {!readOnly && (
...
//       key={index}
//       className={`p-3 text-left font-medium border-r border-[#2a4d75] last:border-r-0 whitespace-nowrap align-middle
//         ${header.key === "fold" ? "w-40" : header.key === "remark" ? "w-40" : ""}
//       `}
//     >
//       <div className="flex items-center gap-2">
//         {header.label}
...
//                                   key={hIdx}
//                                   rowSpan={rowSpan}
//                                   className="p-3 border-r border-gray-200 align-middle text-center font-medium bg-gray-50"
//                                 >
//                                   {renderCell(
//                                     group[key],
//                                     (e) =>
//                                       handleCellChange(e, groupIdx, null, key),
//                                     key.replace(/_/g, " "),
//                                     key,
//                                     true,
//                                   )}
...
//                                     variant[key],
//                                     (e) =>
//                                       handleCellChange(e, groupIdx, vIdx, key),
//                                     unit,
//                                     key,
//                                     true,
//                                   )}
