...
//   const typeFieldEntry = categoryFields.find((f) => !f.section && /\btype\b/i.test(f.label));
...
// These 4 fields are identical across every category in the excel sheet.
// const COMMON_FIELD_LABELS = ['PRODUCT', 'BUYER NAME', 'PROGRAM', 'COUNTRY'];
const COMMON_FIELD_LABELS = ['BUSINESS', 'PRODUCT', 'CUSTOMER', 'PROGRAM', 'COUNTRY'];
...
// Small reusable chevron used by both dropdown-filter headers below,
// so both boxes always animate/look exactly the same way.
function ChevronIcon({ open, dark }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={`h-4 w-4 shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''} ${dark ? 'text-gray-500' : 'text-white/70'}`}
      fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
    >
      <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
    </svg>
...
// A filter-style dropdown: a button showing the label + a short
// summary of the current selection, which opens a small floating
// panel right below it (not an inline accordion that pushes the rest
// of the page down). Closes when you click anywhere outside it —
// exactly like a typical list/table filter dropdown.
function FilterDropdown({ label, summary, icon, children }) {
  const [open, setOpen] = useState(false);

  return (
    <div className={`border rounded-lg bg-white transition-colors ${open ? 'border-blue-400' : 'border-gray-300'}`}>
      <button
        type="button"
      ...
      {/* In-flow panel (not an absolute overlay) — so opening it pushes
          the rest of the page down instead of floating over it and
          getting clipped by the page's scroll container. Its own
          max-height + scroll only kicks in if the option list itself
          is unusually long, so it never grows large enough to hide
          anything below it. */}
      {open && (
        <div className="border-t border-gray-200 p-3 max-h-72 overflow-y-auto thin-scrollbar">
          {children}
        </div>
      )}
    </div>
...
// A single checkbox row used inside the dropdown panels — plain
// checkbox + label, nothing fancier, since this now lives inside an
// already-boxed floating panel.
function FilterCheckboxRow({ label, checked, onChange, locked }) {
  if (locked) {
    return (
      <div className="flex items-center justify-between px-2.5 py-2 rounded-md">
        <span className="text-sm text-gray-400">{label}</span>
        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-100 px-2 py-0.5 rounded">
          Always Required
        </span>
      </div>
    );
  ...
  return (
    <label className="flex items-center gap-2.5 px-2.5 py-2 rounded-md cursor-pointer hover:bg-gray-50">
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
...
function PackagingSpecForm() {
  ...
  // Display-only summaries for the two dropdown-filter buttons —
  // purely derived from existing state, doesn't affect the payload.
  const procurementSummary = assignedVendorId
    ? (procurementUsers.find((v) => String(v.id) === String(assignedVendorId))?.username || 'Selected')
    : 'Not assigned — click to choose';

  const approvalSelections = [
    ppcApprovalRequired && 'PPC',
    tqmApprovalRequired && 'TQM',
    legalApprovalRequired && 'Legal',
    complianceApprovalRequired && 'Compliance',
    labApprovalRequired && 'Lab',
  ].filter(Boolean);
  const approvalSummary = approvalSelections.length > 0
    ? approvalSelections.join(', ')
    ...
    <div className="p-6 w-full h-full overflow-y-auto thin-scrollbar">
      ...
      {category && (
        <form onSubmit={handleSubmit}>
          ...
          {/* Assign Procurement (left) + Assign for Approval (right) —
              both are now filter-style dropdowns: click the button,
              a small panel opens right below it, click outside to
              close — same pattern as a typical list/table filter. */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5 items-start">

            {/* LEFT — Procurement */}
            <FilterDropdown
              label="Assign Procurement"
              summary={procurementSummary}
              icon={
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
            ...
            >
              <label className="flex items-center gap-2.5 px-2.5 py-2 rounded-md cursor-pointer hover:bg-gray-50">
                <input
                  type="checkbox"
                  checked={!assignedVendorId}
                  onChange={() => setAssignedVendorId('')}
                  className="h-4 w-4 accent-blue-600 rounded"
                />
                <span className="text-sm text-gray-500">Assign later</span>
              </label>
              {procurementUsers.map((v) => (
                <label key={v.id} className="flex items-center gap-2.5 px-2.5 py-2 rounded-md cursor-pointer hover:bg-gray-50">
                  <input
                    type="checkbox"
                    checked={String(assignedVendorId) === String(v.id)}
                    onChange={() => setAssignedVendorId(v.id)}
                    className="h-4 w-4 accent-blue-600 rounded"
                  />
                  <span className="text-sm text-gray-800">{v.username}</span>
                </label>
              ))}
            </FilterDropdown>
