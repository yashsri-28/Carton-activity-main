...
function Table({
  ...
  const getRowId = (row) => row.id ?? row.activity_program_status_id;
  ...
  return (
    <div className="relative">
        ...
        <table className="min-w-[1200px] w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#001f3f] text-white text-sm sticky top-0 z-10">
              {enableSelection && (
                <th className="p-4 w-14 rounded-tl-lg">
              ...
              data.map((row, index) => (
                ...
                >
                  ...
                  {columns.map((col, colIdx) => (
                    <td key={col.key || colIdx} className={`p-4 ${col.className || ''}`}>
                        ...
                        {actions.length > 0 && (
                          <div className="relative">
                            <button
                              ...
                              //   e.stopPropagation();
                              //   setActiveDropdownId(
                              //     activeDropdownId === getRowId(row) ? null : getRowId(row)
                              //   );
                              // }}
                              onClick={(e) => {
                                e.stopPropagation();
      ...
      {/* Global Dropdown Portal-like container */}
      {/* {activeDropdownId && (
        <div
          ref={dropdownRef}
          className="fixed z-[9999] w-40 max-h-[180px] !overflow-auto bg-[#E5E7EB] rounded-2xl shadow-xl overflow-hidden border border-gray-200"
          style={{
        ...
        >
          <div className="p-1 space-y-[1px]">
            {(typeof actions === 'function' ? actions(data.find(r => getRowId(r) === activeDropdownId)) : actions).map((action, actionIdx) => (
              <React.Fragment key={actionIdx}>
                <button
                  onClick={() => {
                    const row = data.find(r => getRowId(r) === activeDropdownId);
                    action.onClick(row);
                    setActiveDropdownId(null);
                  }}
                  className="cursor-pointer w-full flex items-center gap-3 px-3 py-3 hover:bg-white/50 transition-colors group text-left"
                >
                ...
                {actionIdx < actions.length - 1 && (
                  <div className="h-[2px] bg-white w-full mx-auto opacity-70"></div>
                )}
              </React.Fragment>
            ))}
          </div>
