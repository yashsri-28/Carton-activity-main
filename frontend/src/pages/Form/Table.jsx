...
//         case 'col12': // Sample Code
//           // Allow letters, numbers, hyphens (e.g. "SC-101")
//           return /^[a-zA-Z0-9\s\-]*$/.test(value);
...
//                       // 👇 FILE UPLOAD COLUMN (edit mode ke liye)
//                       if (header.type === 'file') {
//                         const file = row[key];
//                         return (
//                           <td key={hIdx} className="p-3 border-r border-gray-100 align-middle text-center">
//                             {readOnly ? (
//                               file ? (
//                                 <span className="text-xs text-gray-600">{file.name || 'Attached'}</span>
//                               ) : (
...
//                                   onChange={(e) => onUpdateCell?.(idx, null, key, e.target.files[0])}
...
//                       // 👇 NAYA BLOCK — ATTACHMENT LIST COLUMN (view mode ke liye, backend se aane wale array ke liye)
//                       if (header.type === 'attachment_list') {
//                         const attachments = row[key];
//                         return (
//                           <td key={hIdx} className="p-3 border-r border-gray-100 align-middle text-center">
...
//               title = { att.description || 'View attachment' }
//                                   >
//                                   View Attachment
//             </a>
//                             ))}
  ...
  const validateInput = (value, key) => {
    ...
    if (title === "Program Specifications") {
      switch (key) {
        ...
        case 'wt_unit':
        case 'gsm':
          // Allow integers and floating numbers (e.g. 12 or 12.5)
          return /^[0-9]*\.?[0-9]*$/.test(value);
                ...
                data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    {headers.map((header, hIdx) => {
                      ...
                      // ATTACHMENT LIST COLUMN (view mode, array from backend)
                      if (header.type === 'attachment_list') {
                        const attachments = row[key];
                        return (
                          <td key={hIdx} className="p-3 border-r border-gray-100 align-middle text-center">
