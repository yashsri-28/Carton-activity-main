...
const SelectField = ({ label, name, value, onChange, options, placeholder }) => (
  <div className="flex flex-col gap-2 w-full">
    ...
    <select
      ...
      value={value || ""}
      onChange={onChange}
      className="w-full border border-gray-200 rounded-md p-3 text-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-sm bg-white"
    >
...
const TOWEL_SIZE_OPTIONS = ["Bath", "Hand", "Washcloth", "Bath Sheet", "Mat"];

// Fields that exist ONLY in the Terry Towel workflow — merged into the
// default Towel form so a separate "Terry Towel" category is not needed.
//
// Hidden per client review (kept in formData/backend, just not shown here):
// - Towel Dimensions
// - Towel Weight per Piece
// - Folding Details (this one)
// - Required Pieces per Polybag / per Carton / Polybags per Carton
function TerrySpecificFields({ formData, onInputChange }) {
  return (
    <>
      <FormItem>
